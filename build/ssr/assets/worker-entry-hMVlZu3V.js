...
const requestHandler = createRequestHandler(
  // @ts-ignore
  () => import("./server-build-_IOLc-DE.js"),
  "production"
);
const app = {
  async fetch(request, env2, ctx) {
    ...
    if (url.pathname.startsWith("/api/ci-data")) {
      return handleCIData(request, env2);
    }
    if (url.pathname.startsWith("/api/workflow-runs")) {
      return handleWorkflowRuns(request, env2);
    }
    if (url.pathname.startsWith("/api/job-logs")) {
      return handleJobLogs(request, env2);
    }
...
async function handleCIData(request, env2) {
  try {
    const url = new URL(request.url);
    ...
    const runsResponse = await fetch(
      `https://api.github.com/repos/cloudflare/workers-sdk/actions/runs?per_page=${limit}&status=completed`,
      {
        headers: {
          ...
          ...env2.GITHUB_TOKEN ? { "Authorization": `Bearer ${env2.GITHUB_TOKEN}` } : {}
        }
      }
    );
    if (!runsResponse.ok) {
    ...
    for (const sha in runsByCommit) {
      const commitRuns = runsByCommit[sha].sort(
    ...
    for (const run of runs) {
      ...
      for (const job of jobs) {
          ...
          for (const step of job.steps) {
            ...
            if (stepName.includes("turbo") || stepName.includes("build") || stepName.includes("test")) {
              if (step.started_at && step.completed_at) {
                const stepDuration = (new Date(step.completed_at).getTime() - new Date(step.started_at).getTime()) / 1e3;
                if (stepDuration < 5 && stepDuration > 0) {
                  isCached = true;
                }
              }
            }
            if ((stepName.includes("retry") || stepName.includes("attempt")) && step.conclusion === "success") {
        ...
        if (job.started_at && job.completed_at && !isCached) {
          const duration = (new Date(job.completed_at).getTime() - new Date(job.started_at).getTime()) / 1e3;
    ...
    for (const key in jobsByCommitAndName) {
      const jobs = jobsByCommitAndName[key].sort(
...
async function handleWorkflowRuns(request, env2) {
  try {
    const url = new URL(request.url);
    const limit = parseInt(url.searchParams.get("limit") || "50");
...
async function handleJobLogs(request, env2) {
  try {
    const url = new URL(request.url);
